package com.example.yuuuuu.how_to_go_algo;
...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        final Pathway pathCarCost = new Pathway(cntInput);  //최소비용 자동차
        final Pathway pathCarTime = new Pathway(cntInput);  //최단거리 자동차
        final Pathway pathTrans = new Pathway(cntInput);    //최단거리 대중교통
        ...
        btnCar.setOnClickListener(new View.OnClickListener() {
            public void onClick(View v) {
                ...
                if(cntInput==1) {
                    ...
                    FindLctResult.findCarFree(decidedStartName[0], 0, decidedFinishName[0], 1, urlCarF, pathCarCost);
                    FindLctResult.findCarShort(decidedStartName[0], 0, decidedFinishName[0], 1, urlCarS, pathCarTime);
                }
                ...
                for(int i=0; i<4; i++) {
                    if(decidedStopName[i]!=null) {
                        ...
                        FindLctResult.findCarFree(decidedStartName[0], 0, decidedStopName[i], (i+1), urlCarF, pathCarCost);
                        FindLctResult.findCarShort(decidedStartName[0], 0, decidedStopName[i], (i+1), urlCarS, pathCarTime);
                    }
                    else {
                        break;
                    }
                ...
                for(int i=0; i<3; i++) {
                    for(int j=i+1; j<4; j++) {
                        if(decidedStopName[i]!=null && decidedStopName[j]!=null) {
                            ...
                            FindLctResult.findCarFree(decidedStopName[i], (i+1) , decidedStopName[j], (j+1), urlCarF, pathCarCost);
                            FindLctResult.findCarShort(decidedStopName[i], (i+1), decidedStopName[j], (j+1), urlCarS, pathCarTime);
                        }
                        else {
                            break;
                        }
                ...
                for(int i=0; i<4; i++) {
                    if(decidedStopName[i]!=null) {
                        ...
                        FindLctResult.findCarFree(decidedStopName[i], i+1 , decidedFinishName[0], cntInput, urlCarF, pathCarCost);
                        FindLctResult.findCarShort(decidedStopName[i], i+1, decidedFinishName[0], cntInput, urlCarS, pathCarTime);
                    }
                    else {
                        break;
                    }
                ...
                /* 최소비용, 최단거리 계산 */
                int totCostCar = pathCarCost.cheapestCarCost(); //최소비용 -자동차
                int totCostTaxi = pathCarCost.totalTaxiCost();  //최소비용 -택시
                int totCostCarTime = pathCarCost.totalTime();  //최소비용 -자동차시간
                int totTimeCar = pathCarTime.shortestTime();    //최단거리 -자동차
                int totTimeCarCost = pathCarTime.totalCarCost();    //최단거리 -자동차비용
                int totTimeTaxi = pathCarTime.totalTaxiCost();  //최단거리 -택시비용

                /* 최소비용, 최단거리 전체 경로 */
                String totPathCost = pathCarCost.getPathway();
                String totPathTime = pathCarTime.getPathway();
            }
        });
        ...
        btnBus.setOnClickListener(new View.OnClickListener() {
            public void onClick(View v) {
                ...
                if(cntInput==1) {
                    ...
                    FindLctResult.findBusShort(decidedStartName[0], 0, decidedFinishName[0], 1, urlBusS, pathTrans);
                }
                ...
                for(int i=0; i<4; i++) {
                    if(decidedStopName[i]!=null) {
                        ...
                        FindLctResult.findBusShort(decidedStartName[0], 0, decidedStopName[i], (i+1), urlBusS, pathTrans);
                    }
                    else {
                        break;
                    }
                ...
                for(int i=0; i<3; i++) {
                    for(int j=i+1; j<4; j++) {
                        if(decidedStopName[i]!=null && decidedStopName[j]!=null) {
                            ...
                            FindLctResult.findBusShort(decidedStopName[i], (i+1), decidedStopName[j], (j+1), urlBusS, pathTrans);
                        }
                        else {
                            break;
                        }
                ...
                for(int i=0; i<4; i++) {
                    if(decidedStopName[i]!=null) {
                        ...
                        FindLctResult.findBusShort(decidedStopName[i], (i+1), decidedFinishName[0], cntInput, urlBusS, pathTrans);
                    }
                    else {
                        break;
                    }
                ...
                /* 최단거리 계산 */
                int totTime = pathTrans.shortestTime();

                /* 최단거리 전체 경로 */
                String totPath = pathTrans.getPathway();
            }
        });
    }
